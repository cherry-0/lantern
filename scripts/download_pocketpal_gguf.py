#!/usr/bin/env python3
"""
Download a GGUF model for PocketPal AI and optionally wire it into .env.

PocketPal's README says the app can browse Hugging Face GGUF repos directly and
load any local GGUF file that fits the device. For Verify's native runner, the
required setup is the same: download one GGUF file locally and point
POCKETPAL_GGUF_MODEL_PATH at it.

Examples:
  python scripts/download_pocketpal_gguf.py \
...
DEFAULT_OUT_DIR = REPO_ROOT / "models" / "pocketpal-ai"
DEFAULT_ENV_FILE = REPO_ROOT / ".env"
HF_BASE = "https://huggingface.co"
HF_API_BASE = "https://huggingface.co/api"
ENV_KEY = "POCKETPAL_GGUF_MODEL_PATH"


def _headers() -> dict[str, str]:
    headers = {"User-Agent": "Lantern-PocketPal-GGUF-Downloader/1.0"}
...
def _score_gguf_name(name: str) -> tuple[int, int, int, str]:
    ...
    size_hint = 0
    match = re.search(r"(\d+(?:\.\d+)?)B", upper)
    if match:
        try:
            size_hint = int(float(match.group(1)) * 10)
        except ValueError:
            size_hint = 0
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Download a GGUF model for PocketPal AI and optionally update .env."
    )
    src = parser.add_mutually_exclusive_group(required=True)
    src.add_argument(
        "--repo",
        help="Hugging Face repo id, e.g. bartowski/Qwen2.5-1.5B-Instruct-GGUF",
    )
    ...
    parser.add_argument(
        "--set-env",
        action="store_true",
        help=f"Write {ENV_KEY} to the repo .env file",
    )
    parser.add_argument(
        "--env-file",
    ...
    parser.add_argument(
        "--print-env",
        action="store_true",
        help=f"Print the {ENV_KEY} line after download",
    )
    parser.add_argument(
        "--list-only",
        action="store_true",
    ...
    try:
        if args.repo:
            ...
            selected = args.file or _pick_gguf_file(gguf_files, args.match)
            url = _build_resolve_url(args.repo, selected)
            filename = Path(selected).name
        else:
            if args.list_only:
                raise RuntimeError("--list-only requires --repo.")
            url = args.url
            filename = args.file or _infer_filename_from_url(url)

        if not filename.lower().endswith(".gguf"):
            raise RuntimeError(f"Selected file is not a .gguf: {filename}")
        ...
        if args.set_env:
            _upsert_env_line(Path(args.env_file), ENV_KEY, resolved_dest)

        print(resolved_dest)
        if args.print_env or args.set_env:
            print(f"{ENV_KEY}={resolved_dest}")
        return 0
