...
| 8 | Build-time Secret Embedding | `data/viewmodel/GenAIViewModel.kt` | 234–252 | Module initialization | The Gemini API key is loaded from `BuildConfig.MAPS_API_KEY` and a hardcoded fallback key `AIzaSyAessDw1RJqZWtxUnx0nHTU7V7xxjGv9dg` appears in a comment, baked into the APK at build time. | `private val genAIModel = GenerativeModel(modelName = "gemini-2.5-flash-preview-05-20", apiKey = BuildConfig.MAPS_API_KEY)` and `// in case it not working, please use AIzaSyAessDw1RJqZWtxUnx0nHTU7V7xxjGv9dg` | High |
